...
[目的]
  ...
  前年（Y-1）の市町村平均収量をベースラインとして差し引き、
  目的変数を「絶対収量」から「偏差」に変更して LightGBM LOYO を実施する。
...
[バイアス補正]
  各LOYOフォールドで訓練データの y_diff 平均（mu_train）を引いてゼロ中心化し
  予測時に足し戻すことで、データセットと市町村平均の系統的ズレを補正する。
...
# ── 1. 収量・位置データ ──────────────────────────────────────────────────────
print('=' * 60)
print('  大豆収量偏差予測モデル（農水省前年平均ベースライン）')
print('=' * 60)
...
# join用: MAFFの year を +1 して、「翌年の圃場データ」に結合できるように変換
maff_prev = maff_valid[['year', 'city', 'maff_yield']].copy()
maff_prev['join_year'] = maff_prev['year'] + 1  # 前年のデータを翌年のqueryに当てる
...
# ── 8. LightGBM モデル定義 ────────────────────────────────────────────
def make_lgb_pipe(params: dict = None):
    if params is None:
        params = {}
    return Pipeline([
...
# ── 8b. Optuna ハイパーパラメータチューニング ─────────────────────────────────
N_TRIALS = 100  # 探索試行数

def _hpo_objective(trial, X_all, y_diff_all, y_true_all, y_maff_all, year_all):
    ...
    for test_year in test_years:
        ...
        mu   = float(y_diff_all[tr].mean())
        pipe.fit(X_all[tr], y_diff_all[tr] - mu)
        pred_abs = pipe.predict(X_all[va]) + mu + y_maff_all[va]
...
# ── 9a. Optuna ハイパーパラメータ探索 ─────────────────────────────────────────
print(f'\n{"=" * 60}')
print(f'  Optuna ハイパーパラメータ探索 ({N_TRIALS} trials)')
print(f'  評価方法: LOYO年グループCV + フォールド内平均補正')
print(f'{"=" * 60}')
study = optuna.create_study(direction='minimize',
                            sampler=optuna.samplers.TPESampler(seed=RANDOM_STATE))
study.optimize(
    lambda trial: _hpo_objective(trial, X, y_diff, y_true, y_maff, year_arr),
    n_trials=N_TRIALS,
...
for k, v in best_params.items():
    print(f'    {k:<22} = {v}')
# JSON形式でも出力（フィルタに確実に引っかかるよう）
import json
best_params_str = json.dumps(best_params, indent=2)
print(f'  BEST_PARAMS_JSON: {json.dumps(best_params)}')
# ファイルにも保存
bp_path = os.path.join(OUT_DIR, 'best_params_deviation.json')
with open(bp_path, 'w') as f:
    json.dump({'best_rmse': best_rmse, 'best_params': best_params}, f, indent=2)
print(f'  パラメータ保存: {bp_path}')

print(f'\n{"=" * 60}')
print(f'  LOYO 評価（偏差予測モデル＋フォールド内平均補正）')
print(f'  サンプル数: {len(all_data)} 件 / 特徴量: {X.shape[1]} 次元')
print(f'  テスト年: {test_years}')
print(f'  バイアス補正: 各フォールドの訓練データ y_diff 平均を引いてゼロ中心化')
print(f'{"=" * 60}')
...
for test_year in test_years:
    ...
    pipe = make_lgb_pipe(best_params)
    # ── フォールド内平均補正（方法1） ──────────────────────────────────────────
    mu_train = float(y_diff[tr].mean())        # 訓練データの系統バイアス
    y_diff_centered = y_diff[tr] - mu_train    # ゼロ中心化して学習
    pipe.fit(X[tr], y_diff_centered)
    pred_diff_va = pipe.predict(X[va]) + mu_train  # 予測時にバイアスを足し戻す
    # ───────────────────────────────────────────────────────────────────────────

    # 絶対収量に復元（予測偏差 + 前年MAFF市町村平均）
    pred_abs  = pred_diff_va + y_maff[va]
    true_abs  = y_true[va]
    true_diff = y_diff[va]
    ...
    all_maff.extend(y_maff[va].tolist())     # ナイーブベース用
...
print(f'  {"-"*63}')
print(f'  {"ナイーブ（前年市町村平均）":<25} {rmse_naive:>8.3f} {mape_naive:>7.2f}% {r2_naive:>10.4f} {r2_naive_d:>10.4f}')
...
fig.suptitle('LightGBM LOYO: 偏差予測モデル\n（農水省前年市町村平均収量をベースライン）',

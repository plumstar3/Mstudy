# -*- coding: utf-8 -*-
"""
気象データ(e_input)を除外した大豆収量予測モデル
(先行研究のTF1.x検証ロジックをKeras 2.xで再現)
"""
import os
import numpy as np
import pandas as pd
...
class SoybeanDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        # 入力データを辞書形式で準備 (e_inputを削除)
        X_dict = {
            # 'e_input': 削除 (index 0:312)
        ...
        if self.is_training:
            ...
            for i, seq_info in enumerate(batch_seq_info):
                ...
                for j, year in enumerate(years):
                    ...
                    # features[0:312] は e_input なのでスキップ
...
def build_and_compile_model():
    ...
    # [削除] 環境(E)データ用 共有CNNモデル と ラッパーモデル
    
    # 土壌(S)データ用CNNモデル (変更なし)
    ...
    # --- 全ての特徴量を結合し、LSTMに入力 ---
    # e_processed を削除して結合
    merged = layers.Concatenate()([s_processed, p_processed, ybar_input])
    ...
    # inputsから e_input を削除
    model = models.Model(inputs=[s_input, p_input, ybar_input], outputs=[Yhat1, Yhat2])
...
def run_training_and_evaluation():
    ...
    for year in years_for_mean:
        if year in df_mean_features_by_year.index:
            # (392,) の特徴量配列 (e_input部分含むが、Generatorで無視される)
    ...
    model.save("soybean_yield_model_no_weather.keras")
    print("\n モデル訓練完了・保存済み")
    ...
    if len(val_generator) > 0:
        ...
        loaded_model = models.load_model("soybean_yield_model_no_weather.keras")
        print(" モデル読み込み完了")
        ...
        np.savez("prediction_result_no_weather.npz", Y1_true=Y1_test_true, Y1_pred=Y1_pred)
        print(" 予測結果を 'prediction_result_no_weather.npz' に保存しました")
    else:
...
if __name__ == "__main__":
    ...
    print(" 大豆収量予測モデル (気象データ除外版) - 実行スクリプト")

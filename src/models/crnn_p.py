...
管理データ(P)のみを除外し、環境(E)・土壌(S)データと平均収量(Ybar)を使用する。
"""
import os
import numpy as np
import pandas as pd
...
class SoybeanDataGenerator(tf.keras.utils.Sequence):
    """
    Kerasモデルのためのカスタムデータジェネレータ (管理データPなし版)
    ...
    def __getitem__(self, index):
        ...
        # 入力データを辞書形式で準備 (p_input を削除)
        X_dict = {
            'e_input': np.zeros((actual_batch_size, 5, 312)),
            's_input': np.zeros((actual_batch_size, 5, 66)), # <--- 土壌データは残す
            # 'p_input': (削除)
        ...
        if self.is_training:
            ...
            for i, seq_info in enumerate(batch_seq_info):
                ...
                for j, year in enumerate(years):
                    ...
                    X_dict['s_input'][i, j, :] = features[312:378] # 土壌データ割り当て
            ...
            for i, loc_id in enumerate(batch_loc_ids):
                ...
                X_dict['s_input'][i, 4, :] = features[312:378] # 土壌データ割り当て
                # p_input (378-391) はスキップ
...
def build_and_compile_model():
    """
    管理データ(P)用CNNを除いたモデルを構築する。
    """
    # --- 入力層の定義 (p_inputを削除) ---
    e_input = layers.Input(shape=(5, 312), name="e_input")
    s_input = layers.Input(shape=(5, 66), name="s_input") # <--- 残す
    # p_input = (削除)
    ...
    # 土壌(S)データ用CNNモデル (復活、padding='same'で形状エラー回避)
    s_proc_input = layers.Input(shape=(66,), name="s_proc_input")
    # input_shape (66) -> Reshape (6, 11)
    x_s = layers.Reshape((6, 11))(s_proc_input)
    
    # ★修正点★ padding='same'に変更してサイズ縮小を防ぐ
    # 元のTF1コードの構成を維持しつつ、短いシーケンス(6)に対応
    x_s = layers.Conv1D(4, 3, activation='relu', padding='same')(x_s) 
    x_s = layers.AveragePooling1D(2)(x_s) # (6 -> 3)
    x_s = layers.Conv1D(8, 3, activation='relu', padding='same')(x_s) 
    x_s = layers.Conv1D(12, 2, activation='relu', padding='same')(x_s)
    
    s_cnn_out = layers.Flatten()(x_s)
    ...
    s_processed = layers.TimeDistributed(s_processor, name="TDD_S_Processor")(s_input) # <--- 残す
    # p_processed = (削除)

    # --- 全ての特徴量を結合し、LSTMに入力 (p_processedを削除) ---
    merged = layers.Concatenate()([e_processed, s_processed, ybar_input]) # <--- 修正 (e + s + ybar)
    ...
    # モデルの定義 (入力から p_input を削除)
    model = models.Model(inputs=[e_input, s_input, ybar_input], outputs=[Yhat1, Yhat2]) # <--- 修正
...
def run_training_and_evaluation():
    ...
    # 保存ファイル名を変更
    model_filename = "soybean_yield_model_no_P.keras"
    ...
    if len(val_generator) > 0:
        ...
        # 結果ファイル名を変更
        result_filename = "prediction_result_no_P.npz"
...
if __name__ == "__main__":
    ...
    print(" 大豆収量予測モデル (TF1.x 検証ロジック再現・管理データPなし) - 総合実行スクリプト")
